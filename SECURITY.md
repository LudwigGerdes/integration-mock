...
| Traffic the mock serves | any mocked call, proxy or base-URL mode | **Yes, redacted** — every intercepted request is appended to `~/.integration-mock/requests.jsonl` with its headers and body, and `integration-mock log` reads the same redacted view from the daemon. Before a line is written or served, credential headers and query parameters, token-shaped strings, JWTs, Basic credentials and the paths in the project config's `redact` list are replaced with `[REDACTED]`. Request and response bodies can still hold personal data, so treat the log as sensitive and delete it when done. |
| Recorded responses | `integration-mock record start`, `verify --patch` | Yes, redacted — `./.integration-mock/packs/<service>/`. Before a route is written, credential headers and query parameters (`Authorization`, `Cookie`, `X-API-Key`, `token`, `access_token`, …), token-shaped strings, JWTs, Basic credentials and the project config's `redact` paths are replaced with `[REDACTED]`. Bodies can still hold personal data; review a recorded pack before committing it. |
